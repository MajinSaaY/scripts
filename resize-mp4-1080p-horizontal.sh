#!/bin/bash
#
# This script is used to resize (horizontally) and compress all mp4 files within a folder. (x264)
#
# HOW TO USE
#
...
for file in $tocom
do
   ffmpeg -i "${file}.mp4" -vcodec libx264 -preset veryfast -crf 23 -vf scale=-1:1080 "${file}-recoded.mp4"
   rm "${file}.mp4"
