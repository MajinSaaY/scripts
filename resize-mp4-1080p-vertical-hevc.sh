#!/bin/bash
#
# This script is used to resize (vertically) and compress all mp4 files within a folder. (x265)
#
# HOW TO USE
#
...
for file in $tocom
do
   ffmpeg -i "${file}.mp4" -vcodec libx265 -preset ultrafast -crf 23 -vf scale=1080:-1  "${file}-recoded.mp4"
   rm "${file}.mp4"
